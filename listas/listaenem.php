...
<div id="a4">
    ...
    <div id="title">
        ...
        ?>
        <?php echo $vetor[1];?>ª série <?php echo strtoupper($vetor[2]);?> - Inscrição e Senha do ENEM
    </div>
    <table border="1" width="1000px" cellspacing=0 cellpadding=2 style="font-family: 'Trebuchet MS'; font-variant: small-caps;">
        ...
        while ($loop = mysql_fetch_array($listagem, MYSQL_ASSOC)) {
            $ida = $loop['id'];
            $pegaenem = mysql_query("SELECT * FROM enem WHERE id = '$ida'") or die (mysql_error());
            $linha = mysql_fetch_array($pegaenem);
            $inscricao =  $linha['inscricao'];
            $senha = utf8_encode($linha['senha']);
            ?>
            <tr>
                ...
                <td width="25%"><?php echo $inscricao; ?></td>
                <td width="25%"><?php echo $senha; ?></td>
            </tr>
        <?php
        }
